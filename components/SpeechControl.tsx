...
export const SpeechControl: React.FC<SpeechControlProps> = ({ text }) => {
  ...
  const handleSpeak = async () => {
    ...
    setIsPlaying(false); // In a real app we'd track audio duration, here it just re-enables
  };
    ...
    <div className="flex items-center gap-3 mt-4 bg-black/20 p-2 rounded-lg w-full">
       ...
       <div className="flex gap-2">
         <button 
            ...
            className={`px-2 py-1 text-xs rounded border ${persona === 'HEADMASTER' ? 'bg-cyan-900 border-cyan-500 text-white' : 'bg-space-800 border-space-600 text-slate-400'}`}
         >
            Headmaster
         </button>
         <button 
            onClick={() => setPersona('SISTER')}
            className={`px-2 py-1 text-xs rounded border ${persona === 'SISTER' ? 'bg-pink-900 border-pink-500 text-white' : 'bg-space-800 border-space-600 text-slate-400'}`}
         >
            Sister
         </button>
         <button 
            onClick={() => setPersona('REBEL')}
            className={`px-2 py-1 text-xs rounded border ${persona === 'REBEL' ? 'bg-orange-900 border-orange-500 text-white' : 'bg-space-800 border-space-600 text-slate-400'}`}
         >
            Rebel
         </button>
       </div>
